...
$(".verify_account").click(function() {
    ...
    $.ajax({
        url: "php/admin_ajax.php?verify_account",
        method: "post",
        dataType: "json",
        data: { user_id: user_id_v },
        success: function(response) {
            ...
            if (response.status) {
                $(button).hide();
                $(button).siblings(".unverify_account").show();
                $(button).siblings(".unverify_account").attr("disabled", false);
            } else {
                $(button).attr("disabled", false);
                alert("something is wrong,try again after some time");
            }
...
$(".unverify_account").click(function() {
    var user_id_v = $(this).data("userId");
    var button = this;
    $(button).attr("disabled", true);

    $.ajax({
        url: "php/admin_ajax.php?unverify_account",
        method: "post",
        dataType: "json",
        data: { user_id: user_id_v },
        success: function(response) {
            ...
            if (response.status) {
                $(button).hide();
                $(button).siblings(".verify_account").show();
                $(button).siblings(".verify_account").attr("disabled", false);
            } else {
                $(button).attr("disabled", false);
                alert("something is wrong,try again after some time");
            }
